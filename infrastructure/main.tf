 ...
 * ECS Starter Stack
 *
 * Minimal ECS Fargate infrastructure for the Starter tier:
 * - Public subnets only (no NAT gateway)
 * - ALB + ECS Fargate service
 * - ECR repository
 * - Auto-scaling with CPU tracking
 * - No database, cache, or workers
 */
...
resource "aws_appautoscaling_policy" "app_cpu" {
  name               = "${var.name}-${var.environment}-cpu-autoscaling"
